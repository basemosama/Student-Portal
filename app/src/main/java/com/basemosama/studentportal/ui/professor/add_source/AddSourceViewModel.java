...
public class AddSourceViewModel extends AndroidViewModel {
    public MutableLiveData<String> title = new MutableLiveData<>();
    public MutableLiveData<String> path = new MutableLiveData<>();
    public MutableLiveData<Resource<Source>> addSourceResultMutableLiveData;
    ...
    private MutableLiveData<AddAssignmentFormState> sourceFormStateMutableLiveData = new MutableLiveData<>();
    private LiveData<Resource<List<Subject>>> subjectsLiveData;
    private CombinedLiveData<Integer, Integer> combinedLiveData;


    public AddSourceViewModel(@NonNull Application application) {
        super(application);
        ...
        updateSubjects();
        addSourceResultMutableLiveData = repository.getAddSourceResultLiveData();
    ...
    public void uploadNewSource(View view) {
        setUpAssignmentFormState();
        if (sourceFormStateMutableLiveData.getValue() != null && sourceFormStateMutableLiveData.getValue().isDataValid()) {
            repository.addSource(title.getValue(), path.getValue(),
    ...
    private void setUpAssignmentFormState() {
        if (TextUtils.isEmpty(title.getValue())) {
            sourceFormStateMutableLiveData.setValue(new AddAssignmentFormState(R.string.invalid_assignment_title, null, null, null, null));
        } else if (TextUtils.isEmpty(path.getValue())) {
            sourceFormStateMutableLiveData.setValue(new AddAssignmentFormState(null, R.string.invalid_assignment_path, null, null, null));
        } else if (getDepartmentId() == null) {
            sourceFormStateMutableLiveData.setValue(new AddAssignmentFormState(null, null, R.string.invalid_assignment_department, null, null));
        } else if (getGradeId() == null) {
            sourceFormStateMutableLiveData.setValue(new AddAssignmentFormState(null, null, null, R.string.invalid_assignment_grade, null));
        } else if (getSubjectId() == null) {
            sourceFormStateMutableLiveData.setValue(new AddAssignmentFormState(null, null, null, null, R.string.invalid_assignment_subject));
        } else {
            sourceFormStateMutableLiveData.setValue(new AddAssignmentFormState(true));
        }
    }

    public MutableLiveData<AddAssignmentFormState> getSourceFormState() {
        return sourceFormStateMutableLiveData;
    }

    public MutableLiveData<Resource<Source>> getResult() {
        return addSourceResultMutableLiveData;
    }
